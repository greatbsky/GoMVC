...
( function() {
	CKEDITOR.plugins.add( 'uploadimage', {
		...
		init: function( editor ) {
			...
			if ( !uploadUrl ) {
				CKEDITOR.error( 'uploadimage-config.conf' );
				return;
			}

			// Handle images which are available in the dataTransfer.
			//You can use uploadImgSupportedTypes variables in the config.conf.js file type of custom image upload
			fileTools.addUploadWidget( editor, 'uploadimage', {
				supportedTypes: editor.config.uploadImgSupportedTypes == undefined ? /image\/(jpeg|png|gif|bmp)/ : editor.config.uploadImgSupportedTypes,
